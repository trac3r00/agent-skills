...
## OVERVIEW
31 agent skills (SKILL.md standard) in five families; 9 are script-backed stdlib-only Python CLIs with exit-code gates, 22 are instruction-only. Installable as a Claude Code / Codex plugin or used from a clone by any SKILL.md-reading agent.
...
## CONVENTIONS
- CLI contract (all 9 script-backed skills): argparse, positional input or `-` stdin, `--json`, gates are strict `>` comparisons; rc 0 ok / 1 gate breach / 2 usage-or-input error.
...
## COMMANDS
```bash
uvx pytest tests/ -q                 # full suite (39 tests)
