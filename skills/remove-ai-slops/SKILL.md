...
version: 1.0.0
license: MIT (from oh-my-opencode / omo-ai)
metadata:
  agentskills:
    tags: [refactor, code-quality, cleanup, ai-slop]
---

## Harness portability note

Examples below may reference agent-orchestration tools (parallel subagent
dispatch, background task output, team coordination). Translate them to
whatever your harness provides — Claude Code's Task tool, OMO/Senpi's `task`
tool, or plain sequential execution if no subagents exist. Running the cleanup
passes sequentially in one agent is valid; parallelism is an optimization, not
a requirement.
